...
namespace fast_planner
{
    ...
    void SDFMap::initMap(ros::NodeHandle &nh)
    {
        ...
        // 当地图的占用概率低于此值时，可能会将其截断为最小概率
        // 低于该值为未知体素
        nh.param("sdf_map/p_min", mp_->p_min_, 0.12);
        // 当地图的占用概率高于此值时，可能会将其截断为最大概率。
        nh.param("sdf_map/p_max", mp_->p_max_, 0.97);
        // @todo:us 障碍物概率阈值，未知体素更新的初始值
        ...
        // @todo:us 通过将 inflate 设置为非零值，可以标记或记录需要进行膨胀处理的元素。
        ...
        // 点云处理过程中，count_hit_: 点云落在该点的次数（障碍物）; count_miss_: 处理时经过的次数（非障碍物）
     ...
     * occ:
     *  1: 体素本身用于计算
     *  0: 体素超出地图边界/光线最大距离，用最邻近点替代; 光线经过的点
     */
    ...
    /**
     * points 与 point_num 为建图开始所有点云
     * camera_pos 是 depthPose 和 cloudPose 触发时的相机位姿
     */
    ...
    // camera_pt 位姿下获得的 3d 点位姿
    Eigen::Vector3d SDFMap::closestPointInMap(const Eigen::Vector3d &pt, const Eigen::Vector3d &camera_pt)
    {
