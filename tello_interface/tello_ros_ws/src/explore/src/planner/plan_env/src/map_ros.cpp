...
namespace fast_planner
{
    ...
    void MapROS::init()
    {
        ...
        proj_points_.resize(480 * 320 / (skip_pixel_ * skip_pixel_));
        point_cloud_.points.resize(480 * 320 / (skip_pixel_ * skip_pixel_));
        proj_points_cnt_ = 0;
    ...
    //     Eigen::Vector3d angular(pose->twist.twist.angular.x, pose->twist.twist.angular.y, pose->twist.twist.angular.z);
    //     const double yd_ = 45.0 / 180.0 * M_PI;
    //     if (angular.norm() > yd_)
    //     {
    //         return;
    //     }
    ...
    //     proessDepthImage();
    //     map_->inputPointCloud(point_cloud_, proj_points_cnt_, camera_pos_);
    //     if (local_updated_)
    //     {
    ...
    void MapROS::proessDepthImage()
    {
        ...
        for (int v = depth_filter_margin_; v < rows - depth_filter_margin_; v += skip_pixel_)
        {
            ...
            for (int u = depth_filter_margin_; u < cols - depth_filter_margin_; u += skip_pixel_)
            {
                ...
                auto &pt = point_cloud_.points[proj_points_cnt_++];
                pt.x = pt_world[0];
                pt.y = pt_world[1];
                pt.z = pt_world[2];
            }
    ...
    void MapROS::publishMapAll()
    {
        ...
        ofstream file("/home/ncadeaku/Softwares/ViewPlanning/FUEL_sim_grav/resources/volumn.txt", ios::app);
