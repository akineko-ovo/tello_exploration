...
struct MapData {
  ...
  std::vector<double> occupancy_buffer_;    // 障碍物概率log
  ...
  char raycast_num_;
  queue<int> cache_voxel_;  // 点云处理过程中，状态可以更新的点的集合
  Eigen::Vector3i local_bound_min_, local_bound_max_;     // 局部地图更新区域（膨胀后）
  Eigen::Vector3d update_min_, update_max_;     // 局部地图更新区域
