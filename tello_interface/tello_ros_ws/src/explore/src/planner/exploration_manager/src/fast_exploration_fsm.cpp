...
namespace fast_planner
{
    void FastExplorationFSM::init(ros::NodeHandle &nh)
    {
        ...
        fail_pub_ = nh.advertise<std_msgs::Empty>("/planning/fail", 10);
    ...
    void FastExplorationFSM::FSMCallback(const ros::TimerEvent &e)
    {
        ...
        case WAIT_TRIGGER:
        {
            ...
            // if (wait_times == 8) 
            // {
            //     transitState(PLAN_TRAJ, "WAIT_TRIGGER");
            // }
            break;
        }
        ...
        case PLAN_TRAJ:
        {
            ...
            else if (res == FAIL)
            {
                ...
                ROS_WARN("plan fail");
                fd_->static_state_ = true;
                fail_pub_.publish(std_msgs::Empty());
            }
            break;
        }
    ...
    int FastExplorationFSM::callExplorationPlanner()
    {
        ...
        if (res == SUCCEED)
        {
            ...
            // 如果当前时间比time_r早或相等，说明在一个较短的时间间隔内再次进行规划，此时仍然使用之前的起始时间
            // 确保轨迹规划的连续性，使得不同时间点的规划结果能够无缝衔接
    ...
    void FastExplorationFSM::visualize()
    {
        ...
        // Draw graph
        visualization_->drawGraph(ed_ptr->graph_vertices_, ed_ptr->graph_edges_);
    }
    ...
    // 在WAIT_TRIGGER和FINISH状态下更新并显示全地图frontiers
    void FastExplorationFSM::frontierCallback(const ros::TimerEvent &e)
    {
        ...
        if (state_ == WAIT_TRIGGER || state_ == FINISH)
        {
            ...
            // 所有边界: vector<vector<Eigen::Vector3d>>, 边界由所有网格的中心坐标组成
            ft->getFrontiers(ed->frontiers_);
            // vector<pair<Eigen::Vector3d, Eigen::Vector3d>>, 网格平均中心center, 长度scale
