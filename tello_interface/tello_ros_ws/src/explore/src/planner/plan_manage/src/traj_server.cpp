...
// pos, vel, acc, yaw, yaw_dot, jerk
vector<NonUniformBspline> traj_;
// NonUniformBspline 这段轨迹的从开始到结束的时间间隔
double traj_duration_;
// NonUniformBspline 这段轨迹的起始时间
ros::Time start_time_;
// 接收到bspline的id
int traj_id_;
// 轨迹可视化时visualization_msgs::Marker的id值
int pub_traj_id_;

shared_ptr<PerceptionUtils> percep_utils_;

// Info of replan
// 收到第一条bspline后就一直为true
...
void failCallback(std_msgs::Empty msg)
{
    ++fail_times;
    if (fail_times > 3)
    {
        ofstream fail("/home/ncadeaku/Softwares/ViewPlanning/FUEL_sim_grav/resources/fail", ios::app);
        fail << "";
    }
}
...
void finishCallback(std_msgs::Empty msg)
{
    ...
    if (finish_times > 3)
    {
        if (have_record)
        {
            ofstream file("/home/ncadeaku/Softwares/ViewPlanning/FUEL_sim_grav/resources/record.txt", ios::app);

            double len = calcPathLength(traj_cmd_);
            double flight_t = (end_time - start_time).toSec();

            file << "flight time:" << flight_t << ", path length:" << len << ", mean vel:" << len / flight_t << ", energy is:" << energy << std::endl;
            have_record = false;
        }
        ofstream finish("/home/ncadeaku/Softwares/ViewPlanning/FUEL_sim_grav/resources/finish", ios::app);
        finish << "";
    }
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber fail_sub = node.subscribe("planning/fail", 10, failCallback);
