...
PerceptionUtils::PerceptionUtils(ros::NodeHandle& nh) {
  // (0.56125, 32.157242), (0.69222, 39.661267), (0.68901, 39.477348)
...
void PerceptionUtils::getClusterCenter(vector<Vector3d> &cluster_center, const double &cluster_size)
{
    // double cluster_size_z = fmin(2.0 * max_dist_ * sin(top_angle_ / 2.0), cluster_size);
    int center_num = int(ceil(max_dist_ / cluster_size));
    double center_dist = max_dist_ / center_num;
    cluster_center.clear();

    // std::cout << "cluster_size: " << cluster_size << std::endl;

    for (int i = 0; i < center_num; ++i) {
        double dist = (i + 0.5) * center_dist;

        Vector3d left_center(dist * cos(left_angle_), -dist * sin(left_angle_), 0);
        Vector3d right_center(dist * cos(right_angle_), dist * sin(right_angle_), 0);
        Vector3d top_center(dist * cos(top_angle_), 0, dist * sin(top_angle_));
        Vector3d down_center(dist * cos(top_angle_), 0, -dist * sin(top_angle_));

        cluster_center.emplace_back(left_center);
        cluster_center.emplace_back(right_center);
        cluster_center.emplace_back(top_center);
        cluster_center.emplace_back(down_center);
    }

    double delta_angle = (left_angle_ + right_angle_) / 2.0;
    double init_angle = 0;

    if (center_num % 2 == 1) {
        Vector3d front_center(max_dist_ * (1.0 + cos(top_angle_)) / 2.0, 0, 0);
        cluster_center.emplace_back(front_center);
        init_angle = delta_angle / 2.0; 
    }

    center_num /= 2;
    for (int i = 0; i < center_num; ++i) {
        double angle = init_angle + (i + 1) * delta_angle;
        double dist = max_dist_ * (1.0 + cos(top_angle_)) / 2.0;
        Vector3d front_left(dist * cos(angle), -dist * sin(angle), 0);
        Vector3d front_right(dist * cos(angle), dist * sin(angle), 0);

        cluster_center.emplace_back(front_left);
        cluster_center.emplace_back(front_right);
    }
}
} // namespace fast_planner