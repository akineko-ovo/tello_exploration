...
class AutoRacer():
    ...
    def cb_pos_cmd(self, msg):
        q_temp = tf.transformations.quaternion_from_euler(0, 0, msg.yaw).tolist()
    ...
    def set_waypoint(self, wp):
        ...
        try:
            ...
            res = self.update_target_call(wp.pose.position.x, \
                                          ...
                                          wp.pose.position.z, \
                                          yaw)
            rospy.loginfo("POS_CMD: {} {} {} {}".format(wp.pose.position.x, wp.pose.position.y, wp.pose.position.z, yaw))
            return res
